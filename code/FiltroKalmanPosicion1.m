...
d2f=-2*exp(-t).*cos(t)+(0.032*(rand(1,size(t,2))-0.5));%Aceleración contaminada X
d2g=2*exp(-t).*sin(t)+(0.0431*(rand(1,size(t,2))-0.5));%Aceleracion contaminada Y
...
for i=1:size(v,2)
    % Predicción
    F=[eye(6),x(6)*[0;h;-h;h;-h;h];zeros(1,6),lamb*d2f(i)/abs(d2f(i))];
    H=[1,d2f(i)^4,d2f(i)^3,d2f(i)^2,d2f(i),1,0];
    x=F*x;
    P=F*P*F'+Q;
    % Corrección
    K=P*H'*inv(H*P*H'+R);
    y=sum([x(1)*d2f(i)^4,x(2)*d2f(i)^3,x(3)*d2f(i)^2,x(4)*d2f(i),x(5),0]);
    x=x+K*(v(:,i)-H*x+h);
    P=(eye(1)-K*H)*P;;
    das=[das H*x];
end
