...
d2f=-2*exp(-t).*cos(t)+(1.32*(rand(1,size(t,2))-0.5));%Aceleración contaminada X
d2g=2*exp(-t).*sin(t)+(1.431*(rand(1,size(t,2))-0.5));%Aceleracion contaminada Y
...
% for i=1:size(v,2)
%     % Predicción
%     F=[1+2*x(2)+1,h+x(3),(h^2)/2;0,1,h;0,0,1];
%     x=F*x;
%     P=F*P*F'+Q;
%     % Corrección
%     K=P*H'*inv(H*P*H'+R);
