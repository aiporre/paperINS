...
Q=[5e-6*eye(4),zeros(4,3);zeros(3,4),0.8e-14*eye(3)];
%Q=0.17e-12*eye(7);
R=zeros(3,3);
R(1,1)=0.891718;
R(2,2)=0.891718;
...
h=tout(2)-tout(1);
t=tout';
p=vang(:,1)'+0.000533*rand(1,size(t,2))+0.052;
q=vang(:,2)'+0.00087543*rand(1,size(t,2))-0.0713;
r=vang(:,3)'+0.0004243*rand(1,size(t,2))+0.051;

ax=-9.81*Rcmps(:,7)'+0.000613*rand(1,size(t,2))+a(:,1)';
ay=-9.81*Rcmps(:,8)'+0.000713*rand(1,size(t,2))+a(:,2)';
az=-9.81*Rcmps(:,9)'+0.00813*rand(1,size(t,2))+a(:,3)';
...
%    MATRIZ DE ROTACION EN CUATERNIONES
   ...
   psi=atan2(Rtheta(2,1),Rtheta(1,1));
   dd1=[dd1 phi];
   dd2=[dd2 theta];
   dd3=[dd3 psi];
