...
x_real=exp(-t).*cos(t)+1+sin(t);
y_real=exp(-t).*sin(t)+1+sin(t);
z_real=exp(-t).*sin(t)+3+sin(t);

u_real=exp(-t).*(cos(t)-sin(t));
v_real=-exp(-t).*(cos(t)+sin(t));
w_real=-exp(-t).*(cos(t)+sin(t));

ax_real=-2*exp(-t).*cos(t);
ay_real=2*exp(-t).*sin(t);
az_real=2*exp(-t).*sin(t);
for i=1:size(t,2)
...
ax=gB(1,:)+axx+0*0.2613*rand(1,size(t,2));
ay=gB(2,:)+ayy+0*0.1713*rand(1,size(t,2));
az=gB(3,:)+azz+0*0.1813*rand(1,size(t,2));
...
aux2=[];
%%
Q=[0.01e-6*eye(4),zeros(4,3);zeros(3,4),0.8e-14*eye(3)];
%Q=0.17e-12*eye(7);
R=zeros(3,3);
R(1,1)=0.9891718;
R(2,2)=0.9891718;
...
%     uu=[uu w_rads(1)];
  ...
  z=z+(h*w);
  if mod(t(ii),0.5)
      x=0.99*x+0.01*(x_real(ii)+1.7108+0.063*(rand-0.5));
      y=0.99*y+0.01*(y_real(ii)-2.33+0.053*(rand-0.5));
      z=0.99*z+0.01*(z_real(ii)+3.23+0.073*(rand-0.5));
  end
  pos=[x y z]';
  m=w_rads;
  Sm=[0,-m(3),m(2);m(3),0,-m(1);-m(2),m(1),0];
