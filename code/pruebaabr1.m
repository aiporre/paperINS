...
for asd=1:size(t,2)
    medicion(4)=floor(65535*p_cc(asd)/3);
    medicion(5)=floor(65535*q_cc(asd)/3);
    medicion(6)=floor(65535*r_cc(asd)/3);
    medicion(1)=floor(65535*ax(asd)/3);
    medicion(2)=floor(65535*ay(asd)/3);
    medicion(3)=floor(65535*az(asd)/3); 
    for r=1:3
    ...
    db2=[db2 p];
    %---- s -- Hk[1][0]
    Hk(2,1)=0.0025*sqrt(p*p+q*q+r*r)/2;%var: s
    %---I(s)--A[2)[2)
    A(3,3)=cos(Hk(2,1))+0.0025*120.34*(1-xk(1)*xk(1)-xk(2)*xk(2)-xk(3)*xk(3));
    %----Diagonal
    Fk(1,1)=A(3,3)-2*120.34*0.0025*xk(1)*xk(1);
    Fk(2,2)=A(3,3)-2*120.34*0.0025*xk(2)*xk(2);
    Fk(3,3)=A(3,3)-2*120.34*0.0025*xk(3)*xk(3);
    Fk(4,4)=A(3,3)-2*120.34*0.0025*xk(4)*xk(4);
    %----H(s)--A(1,1) y G(s)--Hk(0,0)
    if -0.0000001<Hk(2,1)<0.0000001
       A(2,2)=0.0025/2;
       Hk(1,1)=-0.0025/6;
    else
       A(2,2)=0.0025*sin(Hk(2,1))/(2*Hk(2,1));
       Hk(1,1)=(0.0025/2)*(cos(Hk(2,1))*Hk(2,1)-sin(Hk(2,1)))/(Hk(2,1)*Hk(2,1)*Hk(2,1));
    end 

    %----df/dr
    Fk(2,1)=A(2,2)*p-2*120.34*0.0025*xk(2)*xk(1);
    Fk(3,1)=A(2,2)*q-2*120.34*0.0025*xk(3)*xk(1);
    Fk(4,1)=A(2,2)*r-2*120.34*0.0025*xk(4)*xk(1);

    Fk(1,2)=-A(2,2)*p-2*120.34*0.0025*xk(1)*xk(2);
    Fk(3,2)=-A(2,2)*r-2*120.34*0.0025*xk(3)*xk(2);
    Fk(4,2)=A(2,2)*q-2*120.34*0.0025*xk(4)*xk(2);

    Fk(1,3)=-A(2,2)*q-2*120.34*0.0025*xk(1)*xk(3);
    Fk(2,3)=A(2,2)*r-2*120.34*0.002*xk(2)*xk(3);
    Fk(4,3)=-A(2,2)*p-2*120.34*0.0025*xk(4)*xk(3);

    Fk(1,4)=-A(2,2)*r-2*120.34*0.0025*xk(1)*xk(4);
    Fk(2,4)=-A(2,2)*q-2*120.34*0.0025*xk(2)*xk(4);
    Fk(3,4)=A(2,2)*p-2*120.34*0.0025*xk(3)*xk(4);

    %%%-------df/db

    Fk(1,5)=xk(2)*p+xk(3)*q+xk(4)*r;
    Fk(1,5)=0.0025*(A(2,2)*xk(2)+Hk(1,1)*Fk(1,5))/2;
    ...
    Fk(2,5)=0.0025*(A(2,2)*xk(2)+Hk(1,1)*Fk(2,5))/2;
    ...
    Fk(3,5)=0.0025*(A(2,2)*xk(3)+Hk(1,1)*Fk(3,5))/2;
    ...
    Fk(4,5)=0.0025*(A(2,2)*xk(4)+Hk(1,1)*Fk(4,5))/2;
