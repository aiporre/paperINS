...
p=p_real+0.00533*rand(1,size(t,2))+0.052;
q=q_real+0.0087543*rand(1,size(t,2))-0.0713;
r=r_real+0.004243*rand(1,size(t,2))+0.051;

% x_real=10*exp(-t).*cos(t)+1;%-0.1*2.25*sin(2*t)/2;
% y_real=10*exp(-t).*sin(t)+1;%-0.1*1.5*sin(3.1*t)/(3.1^2);
% z_real=10*exp(-t).*sin(t)+3;%-0.1*2.5*sin(1.5*t)/(1.5^2);
% 
% u_real=10*exp(-t).*(cos(t)-sin(t));%-0.1*2.25*cos(2*t);
% v_real=-10*exp(-t).*(cos(t)+sin(t));%-0.1*1.5*cos(3.1*t)/3.1;
% w_real=-10*exp(-t).*(cos(t)+sin(t));%-0.1*2.5*cos(1.5*t)/1.5;
% 
...
ax=gB(1,:)+axx+0*0.02613*rand(1,size(t,2));
ay=gB(2,:)+ayy+0*0.01713*rand(1,size(t,2));
az=gB(3,:)+azz+0*0.01813*rand(1,size(t,2));
...
pos_GPS=[];
%%
Q=[1.9e7*eye(4),zeros(4,3);zeros(3,4),0.8e-12*eye(3)];
%Q=0.17e-12*eye(7);
R=zeros(3,3);
R(1,1)=0.00000000009891718;
R(2,2)=0.000000000009891718;
R(3,3)=0.00000000000991718;
Pk=0.1*eye(7,7);
xk=[1,0,0,0,0,0,0];
...
%    MATRIZ DE ROTACION EN CUATERNIONES
   ...
   %psi2=psi2+h*w_rads(3);
   %psi=0.7*psi+0.3*psi2;
   %psi2=psi;
...
%     uu=[uu w_rads(1)];
  ...
  y=y+(h*v);
  z=z+(h*w);
  pos_GPS=[pos_GPS GPS_mea];
  if  (mod(ii,20)==1)&&(ii>1)
      x=0.81*x+0.19*(x_real(ii)+x_rand);
      y=0.01*y+0.99*(y_real(ii)+y_rand);
      z=0.80*z+0.10*(z_real(ii)+z_rand);
      GPS_mea=[(x_real(ii)+x_rand);(y_real(ii)+y_rand);(z_real(ii)+z_rand)];
      if (x-x0)>1.5
          x_rand=10*(rand-0.5);
          x0=x;
      end
