...
\end{equation}
The main disadvantage in the formulation of the complementary filters on SO(3) is the sensibility  to $R_y$ in as much it is involved in the mapping of $\omega_s$ to $\marco{A}$, fot that reason the detemination of this matrix play a mayor role. They mention that the optimal solution can be termined in the minimization argument of:
...
%%%%%%%%%%%%%%
%%%%%%%%%%%%%%
\subsection{Rotation matrix from a gravity measument model on quaternions.}
\begin{figure} [t]
\begin{center}
...
En la definición del observador óptimo, se identifica entonces el caso particular de la ecuación \ref{ProblemaOptimizacion}, donde la matriz de rotación óptima $R^*$ depende de la relación de optimización del cuaternión unitario de rotación $\breve{q}$, denotado por $\breve{q}=q_0+q_1i+q_2j+q_3k$ en $\mathbb{H}$:
